...
function isFalsePositiveLongTubeTrigger(item) {
  const normSku = normalizeSku(item?.sku);
  // 50301-0062-* lines are rail end caps. They ship as accessory/ride-along,
  // not as standalone long-tube handling units.
  if (normSku.startsWith('50301-0062-')) return true;
  return false;
}
...
function predictPallets(items, context = {}) {
  ...
  for (const item of items) {
    ...
    const longTubeTriggered =
      (classification === 'long_tube_trigger' || isLongTubeTriggerItem(item)) &&
      !isFalsePositiveLongTubeTrigger(item);

    if (longTubeTriggered) {
